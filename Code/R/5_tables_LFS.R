...
g <- ggplot(data=dynamics_2digit, aes(y=febmar20, x=benchmark)) 
...
g <- ggplot(data=dynamics_2digit, aes(y=febapr20, x=benchmark)) 
g <- g +  geom_point() + geom_smooth(method=lm, colour="black", se=FALSE)
#g <- g + geom_text(aes(label=code),hjust=0, vjust=0, size = 5)
g <- g + theme_bw()
g <- g + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12))
...
g <- ggplot(data=dynamics_2digit, aes(y=marapr20, x=benchmark)) 
g <- g +  geom_point() + geom_smooth(method=lm, colour="black", se=FALSE)
#g <- g + geom_text(aes(label=code),hjust=0, vjust=0, size = 5)
g <- g + theme_bw()
g <- g + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12))
